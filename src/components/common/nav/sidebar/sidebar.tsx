...
import { MenuButton } from "./sidebar_menu";
...
export const Sidebar: FunctionComponent<INavSidebarProps> = ({
  links,
  theme,
  logo,
  type
  ...
  const [ { x }, setX ] = useSpring(() => ({ x: -100, config: DEFAULT_SPRING_CONFIG, immediate: false }));
  ...
  const bind = useGesture(({ first, last, delta, vxvy }) => {
    ...
    if (last) {
      ...
      if (releasedFromGesture) {
        ...
        setOpen(Math.abs(vxvy[0]) > 0.25 ? (vxvy[0] > 0 ? true : false) : sidebarOpen);
      }
      return;
    }
    ...
    if (!releasedFromGesture && gestureActive) {
      ...
      if (sidebarOpen && x.getValue() < -50 ||!sidebarOpen && x.getValue() > -50) {
      ...
      const newX =  Math.max(-100, Math.min(0, gesturePosition + ((delta[0] / sidebarWidth()) * 100) ));
  ...
  const transitionDelay = (index: number, total: number, direction: boolean) => {
  ...
  return (
    <div className={classnames(styles.wrapper, {[styles.fixed]: type === "fixed"})}>
      <div onClick={() => setSidebarOpen(false)} className={classnames(styles.overlay, { [styles.shade]: sidebarOpen })} />
      <animated.div
        className={styles.sidebar}
        style={{
          transform: x.interpolate(x => `translate3d(${x}%,0,0)`),
          boxShadow: x.interpolate(x => `14px 0 28px rgba(0,0,0,${0.2*(x+100)/100}), 10px 0 10px rgba(0,0,0,${0.18*(x+100)/100})`)
        }}
      >

        <MenuButton onClick={() => setSidebarOpen(!sidebarOpen)} active={sidebarOpen} theme={sidebarOpen? "light" : theme } />
        <SidebarImage load={loadImage} />

        <div {...bind()} className={classnames(styles.gestureTrigger, {[styles.disable]: sidebarOpen})} />
        <div {...bind()} className={styles.content}>

        <SmartLink
          className={classnames(styles.logo, styles.stagger, {[styles.staggerIn]: sidebarOpen})}
          style={{ transitionDelay: transitionDelay(0, links.length + 1, sidebarOpen) }}
          link={logo.link}
        >
          {logo.text}
        </SmartLink>
          ...
          {links.map((link, index) => <div
            key={hash(link)}
            className={classnames(styles.link, styles.stagger, {[styles.staggerIn]: sidebarOpen})}
            style={{ transitionDelay: transitionDelay(index + 1, links.length + 1, sidebarOpen) }}
