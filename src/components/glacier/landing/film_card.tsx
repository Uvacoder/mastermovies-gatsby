...
export const GlacierFilmCard: FunctionComponent<IGlacierFilmCardProps> = ({ name, release, description, thumbnails }) => (
  <div className={styles.root}>
    <div className={styles.cover}>
      {generatePicture(thumbnails)}
    </div>
...
function generatePicture(thumbs: IGlacierFilmThumbnail[]): ReactNode {
  ...
  for (const image of thumbs) {
    const stat = { width: image.width, height: image.height, src: image.image_url };
    mime[image.mime] ? mime[image.mime].push(stat) : mime[image.mime] = [stat];
  }

  // Generate sources with srcset
  if (Object.keys(mime).length !== 0) {
    const sources = Object.entries(mime)
      .map(([key, value]) => (<source key={key} type={key} srcSet={value.map(stat => `${stat.src} ${stat.width}w`).join(", ")} />));
    ...
    return (<div className={placeholder} style={{ position: "relative", top: 32 }}>
