...
export const IndexGlacier: FunctionComponent = () => {
  ...
  return (
    <div className={styles.root}>
      ...
      <p className={styles.releases}>
        New releases
      </p>

      <div className={styles.filmsContainer}>
        <FilmCardResolver film={films && films[0] ? films[0].fingerprint : null} error={filmsError} onRetry={() => setFilmsError(false)} />
        <Icon type="minus" style={{margin: "16px", color: "#AAA"}}/>
        <FilmCardResolver film={films && films[1] ? films[1].fingerprint : null} error={filmsError} onRetry={() => setFilmsError(false)} />
      </div>
